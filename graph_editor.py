...
class App(pyglet.window.Window):
    def __init__(self):
        super(App, self).__init__(800, 600, "VertX Graph Analyzer", resizable=True)
        ...
        self.history = [] #all moves till now
        self.history_index = -1
        self.sidebar_width = 300

        # create vertex list
        self.statusbar = pyglet.graphics.vertex_list(4,
            ('v2f', (0, 0, self.width, 0, self.width, 24, 0, 24)), #vetrex coordinates with 2 floating numbers
            ('c3B', (30, 30, 30) * 4) #vertex color is set as 3 unsigned bits
        )
        ...
        if symbol == key.H:
            ...
            for node_r in self.g.nodes():
                for node_c in self.g.nodes():
                    ...
                    if edges in self.g.edges() or in_edges in self.g.edges(): #adjacency matrix
                        out = out + str(" 1 ")
